...
func NewQueue(queueID int64, capacity uint32, isSingleProducer bool, consumer disruptor.Consumer) *Queue {
	q := new(Queue)
	q.Buffer = NewBuffer(capacity)
	q.IsSingleProducer = isSingleProducer
	//// Build() = single producer vs BuildShared() = multiple producers
	if isSingleProducer {
		q.controller = disruptor.Configure(queueID, int64(q.Buffer.RingBufferCapacity)).WithConsumerGroup(consumer).Build()
	} else {
		q.sharedController = disruptor.Configure(queueID, int64(q.Buffer.RingBufferCapacity)).WithConsumerGroup(consumer).BuildShared()
	}
	q.Start()
	return q
}
...
type Queue struct {
	...
	sharedController disruptor.SharedDisruptor
}
